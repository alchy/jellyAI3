...
## V2b — generativní odpovědi (experimentální)

Malý **decoder-only transformer** (GPT styl) natrénovaný od nuly, který z nalezené
pasáže + otázky odpověď **vygeneruje** (místo vracení věty doslova). Vyžaduje torch
(`requirements-v2.txt`).

```bash
./jelly wiki && ./jelly index && ./jelly qa   # data
./jelly train-gen                             # natrénuj generátor (tokenizer + model, MPS)
./jelly gen "kdo napsal R.U.R.?"              # zeptej se generativně
./jelly gen                                   # interaktivní generativní prompt
```

Přepínač `--gen` funguje i u `ask`/`repl`; extraktivní answerer zůstává výchozí.
Realistický strop: malý model + málo dat = „gramatický přeskládávač" — odpovědi
jsou plynulejší, ale česká shoda občas ustřelí. Je to výukový příklad podmíněné
generace od nuly, ne produkční model.

## Roadmapa

- **Hotovo:** V1 (retrieval + extraktivní QA), V2a (syntetická QA data),
  V2b (generativní odpovědi od nuly).
- **Později:** větší model / víc dat; obecné znalosti mimo načtené texty
  (větší předtrénovaný model).
