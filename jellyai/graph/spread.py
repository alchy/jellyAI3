...
def _neighbor_weights(tokens, window, tau, use_dep):
    """Váhy sousedství: lineární okno (exp(−d/τ)) + symetrické hrany závislostí."""
    n = len(tokens)
    weights = [{} for _ in range(n)]
    for i in range(n):
        for j in range(max(0, i - window), min(n, i + window + 1)):
            if i != j:
                weights[i][j] = weights[i].get(j, 0.0) + math.exp(-abs(i - j) / tau)
        if use_dep:
...
def spread_field(tokens, *, window=2, tau=1.5, alpha=0.5, steps=3,  # pylint: disable=too-many-arguments
    ...
    weights = _neighbor_weights(tokens, window, tau, use_dep)
    heat = list(base)
